import { App, Editor, EditorPosition, EditorSuggest, EditorSuggestContext, EditorSuggestTriggerInfo, FuzzySuggestModal, ItemView, MarkdownPostProcessorContext, MarkdownRenderChild, MarkdownView, Plugin, WorkspaceLeaf, TFile, Notice, Menu, setIcon, Modal } from 'obsidian';
...
// ── Bib: hover popup render child ─────────────────────────────────────────────
class BibRenderChild extends MarkdownRenderChild {
  private activePopup: HTMLElement | null = null;

  constructor(containerEl: HTMLElement, private readonly plugin: FolgezettelPlugin) {
    super(containerEl);
  }
...
// ── Bib: CM6 editor extension — highlight {{...}} in source / live-preview ────
...
// ── Bib: editor suggest — autocomplete {{...}} from Bibliografía folder ────────
class BibEditorSuggest extends EditorSuggest<TFile> {
  constructor(app: App, private readonly plugin: FolgezettelPlugin) { super(app); }
...
export default class FolgezettelPlugin extends Plugin {
  ...
  async onload() {
    ...
    this.registerMarkdownPostProcessor(this.bibPostProcessor.bind(this));
  ...
  async getBibEntry(key: string): Promise<BibEntry | null> {
    ...
    // Normalise author/authors — YAML may produce a string or an array
    ...
    return {
      ...
      year: typeof fmr['year'] === 'string' || typeof fmr['year'] === 'number'
        ? (fmr['year'] as string | number)
        : undefined,
    };
  }

  private bibPostProcessor(el: HTMLElement, ctx: MarkdownPostProcessorContext): void {
    if (!this.replaceCitations(el)) return;
    ctx.addChild(new BibRenderChild(el, this));
  }
